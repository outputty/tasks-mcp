...
- **console** — The interactive terminal (`tasks-mcp --tui`) over every tracker: it lists in-progress-or-ready work across all of them and edits an item in place, every write an existing MCP tool — a human window on the same graph an agent sees, never a private write path. Needs Node 26.4+ (its renderer's native FFI).
