...
- **console** — The interactive terminal (`tasks-mcp --tui`) over every tracker: it lists in-progress-or-ready work across all of them, redraws itself as work moves (it follows each tracker's `/events` change stream), and edits an item in place, every write an existing MCP tool — a human window on the same graph an agent sees, never a private write path. Needs Node 26.4+ (its renderer's native FFI).
- **tracker** — A running tasks-mcp the console talks to as an MCP client. The console starts one for itself (implicit, first in the list) and adds others by URL, saved to `<cacheDir>/console.yaml`; the local and remote paths are one implementation. (replaces: the server, as the console's list entry)
